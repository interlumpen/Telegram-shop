...
from bot.database.methods.read import (
    _get_all_roles, _get_role_by_id, _get_roles_with_max_perms,
    _count_users_with_role, _select_max_role_id, _check_user as check_user,
    _get_role_id_by_name as get_role_id_by_name,
)
from bot.database.methods.create import _create_role
from bot.database.methods.update import _update_role
from bot.database.methods.delete import _delete_role


class TestRoleCRUDMethods:

    def test_create_role(self):
        role_id = _create_role("MODERATOR", 11)  # USE + BROADCAST + USERS_MANAGE
    ...
    def test_create_role_duplicate_name(self):
        _create_role("DUPROLE", 3)
        result = _create_role("DUPROLE", 5)
    ...
    def test_get_role_by_id_nonexistent(self):
        role = _get_role_by_id(99999)
        assert role is None

    def test_get_roles_with_max_perms_user_only(self):
        roles = _get_roles_with_max_perms(1)  # Only USE permission
        assert len(roles) >= 1
        for r in roles:
            assert r['permissions'] <= 1

    def test_get_roles_with_max_perms_all(self):
        roles = _get_roles_with_max_perms(127)  # All permissions
        assert len(roles) >= 3  # At least USER, ADMIN, OWNER

    def test_get_roles_with_max_perms_includes_custom(self, role_factory):
        role_factory("HELPER", 3)
        roles = _get_roles_with_max_perms(127)
        names = [r['name'] for r in roles]
        assert 'HELPER' in names

    def test_count_users_with_role_empty(self):
        role_id = get_role_id_by_name('ADMIN')
        count = _count_users_with_role(role_id)
        assert count == 0

    def test_count_users_with_role_with_user(self, user_factory):
        user_factory(telegram_id=700001, role_id=1)
        user_role_id = get_role_id_by_name('USER')
        count = _count_users_with_role(user_role_id)
        assert count == 1

    def test_update_role(self, role_factory):
        role_id = role_factory("TOUPDATE", 3)
        success, err = _update_role(role_id, "UPDATED", 7)
    ...
    def test_update_role_duplicate_name(self, role_factory):
        role_factory("EXISTING", 3)
        role_id = role_factory("ANOTHER", 5)
        success, err = _update_role(role_id, "EXISTING", 5)
        assert success is False
        assert "already exists" in err

    def test_update_role_nonexistent(self):
        success, err = _update_role(99999, "GHOST", 1)
        assert success is False
        assert "not found" in err

    def test_delete_role_custom(self, role_factory):
        role_id = role_factory("TODELETE", 3)
        success, err = _delete_role(role_id)
        assert success is True
        assert err is None
        assert _get_role_by_id(role_id) is None

    def test_delete_role_builtin_user(self):
        role_id = get_role_id_by_name('USER')
        success, err = _delete_role(role_id)
        assert success is False
    ...
    def test_delete_role_builtin_admin(self):
        role_id = get_role_id_by_name('ADMIN')
        success, err = _delete_role(role_id)
        assert success is False
        assert "built-in" in err

    def test_delete_role_builtin_owner(self):
        role_id = get_role_id_by_name('OWNER')
        success, err = _delete_role(role_id)
        assert success is False
        assert "built-in" in err

    def test_delete_role_with_users(self, user_factory, role_factory):
        role_id = role_factory("BUSYROLE", 3)
        user_factory(telegram_id=700002, role_id=role_id)
        success, err = _delete_role(role_id)
        assert success is False
        assert "users assigned" in err

    def test_delete_role_default(self):
    ...
    def test_select_max_role_id_returns_highest_perms(self):
        max_id = _select_max_role_id()
        role = _get_role_by_id(max_id)
...
class TestRoleManagementHandlers:
    ...
    async def test_role_create_done(self, make_callback_query, fsm_context):
        ...
        role_id = get_role_id_by_name("NEWROLE")
        assert role_id is not None
    ...
    async def test_role_edit_done(self, make_callback_query, fsm_context, role_factory):
        ...
        role_id = role_factory("EDITABLE", 3)
    ...
    async def test_delete_role_confirm(self, make_callback_query, role_factory):
        ...
        role_id = role_factory("DELETEME", 3)
    ...
    async def test_assign_role_list(self, make_callback_query, user_factory):
        ...
        user_factory(telegram_id=700010, role_id=1)
    ...
    async def test_assign_role_list_owner_protected(self, make_callback_query, user_factory):
        from bot.handlers.admin.role_management_states import assign_role_list
        from bot.database.methods.read import _select_max_role_id

        max_role = _select_max_role_id()
        user_factory(telegram_id=700011, role_id=max_role)
    ...
    async def test_assign_role_perm_denied(self, make_callback_query, user_factory):
        ...
        user_factory(telegram_id=700012, role_id=1)
